...
const emptyForm = { name: '', description: '', price: '', photo_url: '', category_id: '', is_visible: true }
...
export default function ProductsPage() {
  ...
  const openEdit = (p) => {
    setForm({ ...p, category_id: p.category_id || '', price: String(p.price) })
    setEditId(p.id)
    setModal('form')
  }

  const submit = async () => {
    const body = { ...form, price: parseFloat(form.price), category_id: form.category_id || null }
    try {
  ...
  const importFile = async (e) => {
    ...
    try {
      ...
      toast.success(`Импортировано: ${r.data.imported} товаров`)
      load()
    } catch {
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {products.length === 0 && (
              <tr><td colSpan={5} className="px-4 py-10 text-center text-ink-500">Товаров нет</td></tr>
            )}
          </tbody>
        </table>
      </div>
