...
async def order_no_comment(callback: CallbackQuery, state: FSMContext):
    # Читаем корзину ДО очистки state
    data = await state.get_data()
    cart = data.get("cart", {})
    await state.clear()
    await state.update_data(cart={})  # явно сбрасываем корзину
    await _place_order(callback.message, callback.from_user.id, None, cart)
    await callback.answer()
...
async def order_with_comment(message: Message, state: FSMContext):
    ...
    await state.clear()
    await state.update_data(cart={})  # явно сбрасываем корзину
    await _place_order(message, message.from_user.id, message.text, cart)

async def _place_order(message, telegram_id: int, comment: str | None, cart: dict):
    if not cart:
        await message.answer("❌ Корзина пуста. Добавьте товары через /start")
        return
